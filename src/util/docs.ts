...
export async function classRunner(this: Client, interaction: CommandInteraction, className: string, version: string): Promise<unknown> {
    const data = await getVersion(version);
    if (data === null) {
        handleIssue("invalid", interaction, version, false, className, null); return;
    }

    const clazz = data.classes.find(c => c.name.toLowerCase() === className.toLowerCase());
    if (clazz === undefined) {
        handleIssue("invalid_class", interaction, version, false, className, null); return;
    }
...
export async function eventRunner(this: Client, interaction: CommandInteraction, className: string, eventName: string, version: string): Promise<unknown> {
    const data = await getVersion(version);
    if (data === null) {
        handleIssue("invalid", interaction, version, false, className, null); return;
    }
    ...
    if (event === undefined) {
        handleIssue("invalid_event", interaction, version, false, className, eventName); return;
    }
...
export async function classPropertyRunner(this: Client, interaction: CommandInteraction, className: string, propertyName: string, version: string): Promise<unknown> {
    const data = await getVersion(version);
    if (data === null) {
        handleIssue("invalid", interaction, version, false, className, null); return;
    }
    ...
    if (property === undefined) {
        handleIssue("invalid_class_property", interaction, version, false, className, propertyName); return;
    }
...
export async function interfacePropertyRunner(this: Client, interaction: CommandInteraction, interfaceName: string, propertyName: string, version: string): Promise<unknown> {
    const data = await getVersion(version);
    if (data === null) {
        handleIssue("invalid", interaction, version, false, interfaceName, null); return;
    }

    const iface = data.interfaces.find(c => c.name.toLowerCase() === interfaceName.toLowerCase());
    if (iface === undefined) {
        handleIssue("invalid_interface", interaction, version, false, interfaceName, null); return;
    }
    const property = iface.properties.find(p => p.name.toLowerCase() === propertyName.toLowerCase());
    if (property === undefined) {
        handleIssue("invalid_interface_property", interaction, version, false, interfaceName, propertyName); return;
    }
...
export async function methodRunner(this: Client, interaction: CommandInteraction, className: string, eventName: string, version: string): Promise<unknown> {
    const data = await getVersion(version);
    if (data === null) {
        handleIssue("invalid", interaction, version, false, className, null); return;
    }
    ...
    if (method === undefined) {
        handleIssue("invalid_method", interaction, version, false, className, eventName); return;
    }
...
export async function typeRunner(this: Client, interaction: CommandInteraction, name: string, version: string): Promise<unknown> {
    const data = await getVersion(version);
    if (data === null) {
        handleIssue("invalid", interaction, version, false, name, null); return;
    }
    ...
    if (type === undefined && iface === undefined) {
        handleIssue("invalid_type", interaction, version, false, name, null); return;
    }
...
export async function handleAutocomplete(this: Client, interaction: AutocompleteInteraction, version: string): Promise<unknown> {
    ...
    if (!check) {
        handleIssue("loading", interaction, version, true, null, null); return;
    }
    ...
    if (root === null) {
        handleIssue("invalid", interaction, version, true, null, null); return;
    }
    ...
    switch (focused.name) {
        ...
        case "event": {
            ...
            if (clazz.events.length === 0) {
                handleIssue("no_events", interaction, version, true, name!, null); return;
            }
        ...
        case "property": {
            ...
            if ((majorType === "class" ? clazz : iface)!.properties.length === 0) {
                handleIssue(`no_properties_${majorType === "class" ? "class" : "interface"}`, interaction, version, true, name!, null); return;
            }
        ...
        case "method": {
            const clazz = root.classes.find(c => c.name === name)!;
            if (clazz.methods.length === 0) {
                handleIssue("no_methods", interaction, version, true, name!, null); return;
            }
