...
export default class DegeneratesCommand extends Command {
    ...
    async add(this: Client, interaction: CommandInteraction, user: Member): Promise<unknown> {
    ...
    async remove(this: Client, interaction: CommandInteraction, user: Member): Promise<unknown> {
        if (!user.roles.includes(role)) {
