...
export default function processProperty(data: JSONOutput.DeclarationReflection): Property | undefined {
    if (!data.type) {
        GenerationLogs.addCurrent(`Attempted to parse reflection ${formatReflection(data)} with no type: ${JSON.stringify(data)}`);
        return;
    }
    if ("name" in data.type && data.type.name === "default" && "id" in data.type && data.type.id !== undefined) {
